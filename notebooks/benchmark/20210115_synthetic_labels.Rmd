...
run_cydar <- function(sce, condition_col="synth_labels",
  ...
  cd <- countCells(cd, tol=r, filter=0, downsample=10)
...
How many subpopulations were detected?
```{r}
data.frame(colData(embryo_sce)[c('true_labels', "true_DA_clust", "milo_res")]) %>%
  mutate(TP= ifelse(true_labels==milo_res & milo_res!="NotDA", 1, 0)) %>%
  mutate(total_TPR = sum(TP)/sum(!is.na(true_DA_clust))) %>%
  group_by(true_DA_clust) %>%
  summarise(TPR=sum(TP)/n(), total_TPR=first(total_TPR)) %>%
  ggplot(aes(as.factor(true_DA_clust), TPR)) + geom_col()
```
...
How many subpopulations were detected?
```{r}
data.frame(colData(embryo_sce)[c('true_labels', "true_DA_clust", "meld_res", "milo_res")]) %>%
  mutate(meld_res=as.character(meld_res)) %>%
  mutate(meld_res=case_when(meld_res=="PosLFC" ~ "NegLFC",
                        meld_res=="NegLFC" ~ "PosLFC",
                        TRUE ~ meld_res)) %>%
  pivot_longer(cols = c("milo_res", "meld_res"), names_to="method", values_to="pred") %>%
  mutate(TP= ifelse(true_labels==pred & pred!="NotDA", 1, 0)) %>%
  group_by(method) %>%
  summarise(TPR_total=sum(TP)/n()) %>%
  ungroup() %>%
  group_by(method, true_DA_clust) %>%
  summarise(TPR=sum(TP)/n()) %>%
  ggplot(aes(x=method,  TPR)) +
  geom_boxplot() +
  geom_point(size=2) +
  geom_line(aes(group=as.factor(true_DA_clust)))
```
...
bm_outcome <- lapply(list.files(outdir, pattern = '.csv', full.names = TRUE), function(x) read_csv(x)) %>%
  purrr::reduce(bind_rows) %>%
...
bm_confusion <-
  lapply(list.files(outdir, pattern = '.csv', full.names = TRUE), function(x) read_csv(x)) %>%
  purrr::reduce(bind_rows) %>%
  ## Flip results for DAseq and MELD
  mutate(pred=case_when(method %in% c("meld", "daseq") & pred=="PosLFC" ~ "NegLFC",
                        method %in% c("meld", "daseq") & pred=="NegLFC" ~ "PosLFC",
...
pl_ls <- lapply(c("TPR", "FPR", "Accuracy", "Power"), function(metric){
  bm_outcome %>%
...
wrap_plots(pl_ls, nrow=2, ncol=2, guides="collect")
```
...
## Simulate batch effects of different magnitude
bm_sce_ls <- lapply(c(0, 0.1, 0.3, 0.5, 0.7, 1), function(sd){
  sce_be <- add_batch_effect(embryo_sce, batch_col = "synth_batches", norm_sd=sd)
  sce_be$norm_sd <- sd
  sce_be
  })
...
```{r}
outdir <- "/nfs/team205/ed6/data/milo_benchmark/"
bm_out <- lapply(list.files(outdir, pattern = '2023.+.csv', full.names = TRUE), function(x) read_csv(x)) %>%
  purrr::reduce(bind_rows)

embryo_sce$milo_res = bm_out[bm_out$batch_sd==0 & bm_out$method=="milo",][["pred"]]
embryo_sce$meld_res = bm_out[bm_out$batch_sd==0 & bm_out$method=="meld",][["pred"]]
embryo_sce$daseq_res = bm_out[bm_out$batch_sd==0 & bm_out$method=="daseq",][["pred"]]
embryo_sce$louvain_res = bm_out[bm_out$batch_sd==0 & bm_out$method=="louvain",][["pred"]]
embryo_sce$true = bm_out[bm_out$batch_sd==0 & bm_out$method=="louvain",][["true"]]

plotReducedDim(embryo_sce, dimred="umap_batch", colour_by="milo_res", point_size=0.2) 
plotReducedDim(embryo_sce, dimred="umap_batch", colour_by="meld_res", point_size=0.2) 
plotReducedDim(embryo_sce, dimred="umap_batch", colour_by="daseq_res", point_size=0.2) 
plotReducedDim(embryo_sce, dimred="umap_batch", colour_by="true", point_size=0.2) 

plotReducedDim(embryo_sce, dimred="pca_batch", colour_by="Condition1_prob", point_size=0.2) 
```
...
bm_confusion <-
  lapply(list.files(outdir, pattern = '2023.+.csv', full.names = TRUE), function(x) read_csv(x)) %>%
  purrr::reduce(bind_rows) %>%
  ## Flip results for DAseq and MEL, 'daseq'
  mutate(pred=case_when(method %in% c("meld", 'daseq') & pred=="PosLFC" ~ "NegLFC",
                        method %in% c("meld", 'daseq') & pred=="NegLFC" ~ "PosLFC",
                        TRUE ~ pred)) %>%
  group_by(method, batch_sd, pred, true) %>%
...
```{r, fig.width=10, fig.height=8, warning=FALSE}
pl_ls <- lapply(c("TPR", "FPR", "Power", "Accuracy"), function(metric){
  bm_outcome %>%
  separate(col = "method", into = c("method", "be_sd"), sep = "_") %>%
  rename(metric=metric) %>%
  ggplot(aes(as.numeric(be_sd), metric, color=method)) +
...
purrr::reduce(milo_res_ls, bind_rows) %>%
  ...
  calculate_outcome() %>%
  separate(col = "method", into = c("method", "be_sd"), sep = "_") %>%
  ggplot(aes(as.numeric(be_sd), Recall, color=method)) +
  geom_point(size=3) +
  geom_line() +
  xlab('batch effect SD') +
  theme_bw(base_size = 18)
