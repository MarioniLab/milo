...
n.dim <- 15
k <- 10
sim.data <- readRDS("~/Dropbox/Milo/simulations/data/Trajectory_Ncells2000_3M1DARep15_simpleSim.RDS")
sim.mylo <- sim.data$mylo
sim.meta <- sim.data$meta
nhoodAdjacency(sim.mylo) <- matrix(0L, ncol=1, nrow=1)
sim.mylo <- buildGraph(sim.mylo, k=k, d=n.dim)
sim.mylo
```
...
sim.mylo <- calcNhoodDistance(sim.mylo, d=n.dim)
...
pdf("~/Dropbox/Milo/figures/MethodCompare_DAseq_pred.pdf", height=4.15, width=5.15, useDingbats=FALSE)
sim.daseq$pred.plot
dev.off()
```
...
```{r, warning=FALSE, fig.height=4.15, fig.width=5.15}
pdf("~/Dropbox/Milo/figures/MethodCompare_DAseq_DAcells.pdf", height=4.15, width=5.15, useDingbats=FALSE)
...
pdf("~/Dropbox/Milo/figures/MethodCompare_DAseq_DAregions.pdf", height=4.15, width=5.15, useDingbats=FALSE)
...
rank.cols <- rev(scico(length(levels(da.negrank.melt$value)), palette='hawaii'))
names(rank.cols) <- levels(da.negrank.melt$value)

da.negrank.melt$Method <- factor(da.negrank.melt$Method,
                                 level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))
...
da.posrank.melt$Method <- factor(da.posrank.melt$Method,
                                 level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))
...
allrank.melt$Method <- factor(allrank.melt$Method,
                              level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))

allmeasure.melt <- melt(da.fdr.df, id.vars=c("Method"))
allmeasure.melt$Method <- factor(allmeasure.melt$Method,
                                 level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))


ggplot(allrank.melt, 
       aes(x=Method, y=variable)) +
    geom_tile(aes(fill=value)) +
    geom_text(data=allmeasure.melt[!allmeasure.melt$variable %in% c("MCC"), ], 
              aes(label=round(value, 2)), colour='white', size=5) +
    theme_cowplot() +
    scale_fill_manual(values=rank.cols) +
    labs(x="Method", y="Measure") +
     theme(axis.text=element_text(size=18),
...
These comparisons can also be extended to multiple runs. How consistently does `Milo` perform compared to these other methods?

```{r, warning=FALSE, message=FALSE}
multi.compare <- readRDS("~/Dropbox/Milo/Methods_comparison_MethodCompare.RDS")
confuse.df <- multi.compare$confuse.df

confuse.df$PPV[is.na(confuse.df$PPV)] <- 0
confuse.df$NPV[is.na(confuse.df$NPV)] <- 1
confuse.df$FDR[is.na(confuse.df$FDR)] <- 1
confuse.df$FPR[is.na(confuse.df$FPR)] <- 1
confuse.df$FNR[is.na(confuse.df$FNR)] <- 1
confuse.df$FOR[is.na(confuse.df$FOR)] <- 1
confuse.df$TPR[is.na(confuse.df$TPR)] <- 0
confuse.df$TNR[is.na(confuse.df$TNR)] <- 0
confuse.df$F1[is.na(confuse.df$F1)] <- 0
confuse.df$MCC[is.na(confuse.df$MCC)] <- 0
confuse.df$Power[is.na(confuse.df$Power)] <- 0
```

I'll compute the ranks for each of the 100 simulations.

```{r, warning=FALSE, message=FALSE}
unique.runs <- unique(confuse.df$FileID)
run.ranks.list <- list()

for(x in seq_along(unique.runs)){
    x.run <- unique.runs[x]
    x.df <- confuse.df[confuse.df$FileID %in% x.run, ]
    x.pos <- apply(x.df[, c("FDR", "FPR", "FNR", "FOR")], 2, FUN=function(X) rank(X))
    x.neg <- apply(x.df[, c("Power", "F1", "TNR", "TPR", "NPV", "PPV")], 2, FUN=function(X) rank(-X))
    x.all <- cbind.data.frame(x.pos, x.neg)
    x.all$Method <- x.df$Method
    
    run.ranks.list[[x.run]] <- x.all
}

ranks.run.df <- do.call(rbind.data.frame, run.ranks.list)

ranks.run.df$Method <- factor(ranks.run.df$Method,
                              level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))
```


```{r, warning=FALSE, message=FALSE}
ggplot(ranks.run.df, aes(x=Method, y=Power)) +
    geom_boxplot() +
    theme_cowplot() +
    NULL
```
...
```{r, warning=FALSE, message=FALSE}
ggplot(ranks.run.df, aes(x=Method, y=FDR)) +
    geom_boxplot() +
    theme_cowplot() +
    NULL
```
...
```{r, warning=FALSE, message=FALSE}
confuse.df$Method <- factor(confuse.df$Method,
                            level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))

ggplot(confuse.df, aes(x=Method, y=MCC)) +
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100MCC-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```
...
ggplot(confuse.df, aes(x=Method, y=FDR)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100FDR-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```

```{r, warning=FALSE, message=FALSE}
ggplot(confuse.df, aes(x=Method, y=FOR)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100FOR-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```


```{r, warning=FALSE, message=FALSE}
ggplot(confuse.df, aes(x=Method, y=F1)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100F1-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```

```{r, warning=FALSE, message=FALSE}
ggplot(confuse.df, aes(x=Method, y=FPR)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100FPR-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```


```{r, warning=FALSE, message=FALSE}
ggplot(confuse.df, aes(x=Method, y=TPR)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100TPR-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```


```{r, warning=FALSE, message=FALSE}
ggplot(confuse.df, aes(x=Method, y=PPV)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100PPV-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```
...
ggplot(confuse.df, aes(x=Method, y=Power)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100Power-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```
...
ggplot(confuse.df, aes(x=Method, y=NPV)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100NPV-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```
...
ggplot(confuse.df, aes(x=Method, y=TNR)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100TNR-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```
...
ggplot(confuse.df, aes(x=Method, y=FNR)) +
    geom_boxplot(fill='grey80') +
    theme_cowplot() +
    expand_limits(y=c(0, 1.0)) +
    theme(axis.text=element_text(size=20),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100FNR-boxplot.pdf",
       height=2.95, width=6.95, useDingbats=FALSE)
```


How about the rank of the average values across 100 runs?

```{r, warning=FALSE, message=FALSE}
confuse.melt <- melt(confuse.df, id.vars=c("Method", "FileID"))
# calculate the mean value across 100 simulations - use these for ranking the methods.
methods_grouped <- confuse.melt %>% group_by(Method, variable) %>%
    summarise(Ave=mean(value, na.action='na.rm')) %>%
    pivot_wider(values_from=Ave, names_from=variable)
```


```{r, warning=FALSE, message=FALSE}
# calculate the rank along each column
da.negrank.df <- as.data.frame(apply(methods_grouped[, c("Power", "F1", "TNR", "NPV", "PPV")],
                                     2, FUN=function(X) rank(-X)))
da.negrank.df$Method <- methods_grouped$Method
da.negrank.melt <- melt(da.negrank.df, id.vars="Method")
da.negrank.melt$value <- ordered(da.negrank.melt$value)
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100PosRank_table.pdf",
       height=3.95, width=6.25, useDingbats=FALSE)
```
...
```{r, warning=FALSE, message=FALSE}
# calculate the rank along each column
da.posrank.df <- as.data.frame(apply(methods_grouped[, c("FDR", "FPR", "FNR", "FOR")],
                                     2, FUN=function(X) rank(X)))
da.posrank.df$Method <- methods_grouped$Method
da.posrank.melt <- melt(da.posrank.df, id.vars=c("Method"))
da.posrank.melt$value <- ordered(da.posrank.melt$value)
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100NegRank_table.pdf",
       height=3.95, width=6.25, useDingbats=FALSE)
```
...
allmeasure.melt <- melt(methods_grouped, id.vars=c("Method"))
allmeasure.melt$Method <- factor(allmeasure.melt$Method,
                                 level=c("Milo", "Cydar", "DAseq", "Louvain", "Walktrap"))
...
ggplot(allrank.melt, 
       aes(x=Method, y=variable)) +
    geom_tile(aes(fill=value)) +
    geom_text(data=allmeasure.melt[!allmeasure.melt$variable %in% c("MCC", "TPR"), ], 
              aes(label=round(value, 2)), colour='black', size=5) +
    theme_cowplot() +
    scale_fill_manual(values=rank.cols) +
    labs(x="Method", y="Measure") +
     theme(axis.text=element_text(size=18),
...
ggsave("~/Dropbox/Milo/figures/MethodCompare_100AllRank_table.pdf",
       height=4.15, width=6.95, useDingbats=FALSE)
```
